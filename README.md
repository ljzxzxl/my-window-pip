...
**交互**
- 自动隐藏 + 点击穿透：鼠标移上去浮窗淡出并暂停，可直接操作背后的内容；淡出后**按住 `⌥` 可临时唤回**（恢复不透明并可点击），也可以在菜单栏的浮窗子菜单里关掉
- 淡出后的不透明度可调，5% 一档（默认 35%），设置页 / 浮窗右键菜单 / 菜单栏子菜单三处都能改
- 悬停浮出控制条：暂停、帧率、复位缩放、自动隐藏、静止检测、关闭；鼠标移到图标上**立刻**显示功能说明（提示条画在浮窗内，不会被浮窗遮挡）
...
| 复位缩放 | `Cmd` + 双击 | 鼠标在浮窗上 |
| 平移 | 滚轮 | 放大后 |
| **临时唤回已淡出的浮窗** | 按住 `⌥` | 开了自动隐藏、浮窗淡出并点击穿透时 |

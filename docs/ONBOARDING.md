...
- **拖动是手动实现的**：`isMovableByWindowBackground = false`，由 `PiPContentView` 在 `mouseDragged` 里按位移 `setFrameOrigin`。原因是系统背景拖动会吞掉 `mouseUp`，拿不到干净的单击，而单击要用来「切回源应用」。拖动会经过 `SessionStore` 的多窗口磁吸解析，但只修正位置、不改尺寸；改动手势时要保持跨屏 scale 变化后 retune 的链路。位置刻意不按 App 持久化，否则同一 App 的多个窗口会互相覆盖。
